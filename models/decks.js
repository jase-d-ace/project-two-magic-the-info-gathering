...
decks.findAll = (user_id) => {
    return db.any(`SELECT deck_name, description FROM decks JOIN users ON users.id = decks.user_id WHERE user_id=$1` [user_id])
};
decks.showDeck = (id) => {
...
decks.create = (name, description, user_id) => {
    return db.one(`INSERT INTO decks(deck_name, description, user_id) VALUES($1, $2, $3) returning id`, [name, description, user_id]);
};
